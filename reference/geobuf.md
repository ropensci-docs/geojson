...
- x:
  ...
  (character) a file or raw object for `from_geobuf`, and json string
  for `to_geobuf`
...
- file:
  ...
  (character) file to write protobuf to. if NULL, geobuf raw binary
  returned

- decimals:

  (integer) how many decimals (digits behind the dot) to store for
  numbers

## Value

for `from_geobuf` JSON as a character string, and for `to_geobuf` raw or
file written to disk

## Details

`from_geobuf` uses
[`protolite::geobuf2json()`](https://jeroen.r-universe.dev/protolite/reference/geobuf.html),
while `to_geobuf` uses
[`protolite::json2geobuf()`](https://jeroen.r-universe.dev/protolite/reference/geobuf.html)

Note that protolite expects either a **Feature**, **FeatureCollection**,
or **Geometry** class geojson object, Thus, for `to_geobuf` we check the
geojson class, and convert to a **Feature** if the class is something
other than the acceptable set.

## References

Geobuf is a compact binary encoding for geographic data using protocol
buffers https://github.com/mapbox/geobuf
